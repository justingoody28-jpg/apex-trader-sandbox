...
async function polyBars(ticker, date, key) {
  const res = await fetch(
    `https://api.polygon.io/v2/aggs/ticker/${ticker}/range/1/minute/${date}/${date}?adjusted=true&sort=asc&limit=1000&apiKey=${key}`
  );
...
async function polyAvgVolume(ticker, key) {
  const res = await fetch(
    `https://api.polygon.io/v2/aggs/ticker/${ticker}/range/1/day/${daysAgo(31)}/${daysAgo(1)}?adjusted=true&sort=asc&limit=35&apiKey=${key}`
  );
