...
const SCENARIOS = {
  ...
  D:  { label:'D',  full:'Short -2% / +0.5% (SPY gap ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¤+0.5%)',  dir:'short', gapDir:'up',   gapMin:0.02, gapMax:null,  rvolMin:null, tp:0.020, sl:0.005, be:20.0, color:'#60a5fa' },
  ...
  E4: { label:'E4', full:'Short -5% / -3% (gap ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¥15%)',           dir:'short', gapDir:'up',   gapMin:0.15, gapMax:null,  rvolMin:null, tp:0.050, sl:0.030, be:37.5, color:'#f87171' },
  ...
  G:  { label:'G',  full:'Long +1.5% / -1.5% (gap ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¤-10% RVOLÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¥5x)', dir:'long', gapDir:'down', gapMin:0.10, gapMax:null, rvolMin:5,   tp:0.015, sl:0.015, be:50.0, color:'#fbbf24' },
  H:  { label:'H',  full:'Long +1.5% / -2.5% (gap ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¤-10% RVOLÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¥4x ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¤9:42)', dir:'long', gapDir:'down', gapMin:0.10, gapMax:null, rvolMin:4, tp:0.015, sl:0.025, be:62.5, color:'#fb923c' },
};
...
function qualifyScenarios(gap, rvol, spyGap, spyRecovering) {
  ...
  if(gap>=0.13 && gap<0.15)                    q.push('E3');
  if(gap>=0.15)                                q.push('E4');
  // F: long gap-down -5% to -25%
  if(gap<=-0.05 && gap>-0.25)                  q.push('F');
  // G: honed fade long, gap <= -10%, RVOL >= 5x
  if(gap<=-0.10 && rvol>=5)                    q.push('G');
  // H: panic reversal, gap <= -10%, RVOL >= 4x
...
function getEntryLabel(sc) {
  if (sc.gapDir === 'up') {
    ...
    return 'gap â¥' + (sc.gapMin*100).toFixed(0) + '%' + mx;
  } else {
    const mn = 'gap â¤-' + (sc.gapMin*100).toFixed(0) + '%';
    const mx = sc.gapMax ? ' to -' + (sc.gapMax*100).toFixed(0) + '%' : '';
    const rv = sc.rvolMin ? ' RVOLâ¥' + sc.rvolMin + 'x' : '';
    return mn + mx + rv;
  }
}
export default function BacktestPage() {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      for(let ti=0;ti<TICKERS.length;ti++){
        ...
        let bars;
        try { addLog(`ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ${ticker}...`); bars=await getDailyBars(ticker,FROM,TO); await sleep(150); }
        catch(e){ addLog(`ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ${ticker}: ${e.message}`); continue; }
        if(!bars||bars.length<25){ addLog(`ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ  ${ticker}: no data`); continue; }
        ...
        addLog(`ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ${ticker}: ${n} qualifying days`);

        if(done%CHUNK===0||done===TICKERS.length){
          const isFinal=done===TICKERS.length;
          addLog(`\nÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Interim ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ${done}/${TICKERS.length} tickers (${allTrades.length} trades)\n`);
          updateResults([...allTrades],done,TICKERS.length,isFinal);
        }
      }

      setProg({msg:'Complete',pct:100});
      addLog(`ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Done ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ${allTrades.length} total trades`);
      setTab('results');

    } catch(e){ addLog(`\nÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¥ ${e.message}`); }
    finally{ setRunning(false); }
  },[]);

  // Formatters
  const pct   = v => v===null?'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ':`${(v*100).toFixed(1)}%`;
  const pfmt  = v => v===null?'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ':v===Infinity?'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ':`${v.toFixed(2)}x`;
  const ret   = v => v===null?'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ':`${v>0?'+':''}${(v*100).toFixed(1)}%`;
  ...
  const tabLabel = t => {
    if(t==='results'&&results&&running) return `ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Results (${results.tickersDone}/${results.totalTickers})`;
    if(t==='results'&&results) return 'ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Results ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ';
    return {setup:'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Setup',running:'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶ Running',results:'ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Results'}[t];
  };
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',borderBottom:'1px solid #1f2937',paddingBottom:'12px',marginBottom:'16px'}}>
        <div>
          <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
            <a href="/" style={{color:'#4b5563',fontSize:'11px',textDecoration:'none'}}>ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ APEX</a>
            <div style={{fontSize:'16px',fontWeight:'bold',color:'#f9fafb'}}>ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Signal Correlation Backtester</div>
          </div>
          <div style={{fontSize:'10px',color:'#4b5563',marginTop:'2px'}}>41 tickers ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· 5 signals ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· Mar 2024ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂMar 2025 ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· Scenarios A D E1-E4 F G H</div>
        </div>
        <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
          {['setup','running','results'].map(t=>(
            <button key={t} style={C.tabBtn(tab===t)} onClick={()=>setTab(t)}>{tabLabel(t)}</button>
          ))}
          {running&&<span style={{fontSize:'10px',color:'#fbbf24',marginLeft:'4px'}}>ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ LIVE</span>}
        </div>
      </div>
      ...
      {tab==='setup'&&(
        <div style={{maxWidth:'580px',display:'flex',flexDirection:'column',gap:'14px'}}>
          <div style={{...C.card,borderColor:'#064e3b',background:'#022c22'}}>
            <div style={{fontSize:'11px',color:'#34d399'}}>ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ API keys loaded from environment. Interim results every {CHUNK} tickers.</div>
          </div>
          <div style={C.card}>
            ...
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:'11px'}}>
              <thead>
                <tr style={{borderBottom:'1px solid #1f2937'}}>
                  {['Scenario','Direction','Entry','TP','SL','Breakeven WR','Notes'].map(h=>(
          ...
          <button onClick={run} disabled={running}
            ...
            {running?'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ³ Running...':'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶ Run Backtest ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ 7 Scenarios ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· Results every 10 tickers'}
          </button>
        </div>
      )}
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
          <div style={C.card}>
            ...
            <div style={{height:'400px',overflowY:'auto',fontSize:'11px',lineHeight:'1.7'}}>
              {log.map((l,i)=>(
                <div key={i} style={{color:l.startsWith('ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ')?'#374151':l.startsWith('ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ')?'#7f1d1d':l.startsWith('ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ')?'#3b82f6':l.startsWith('ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ')?'#065f46':'#6b7280'}}>{l}</div>
              ))}
            </div>
          ...
          <div style={C.card}>
            <div style={{fontSize:'10px',color:'#6b7280',marginBottom:'8px',textTransform:'uppercase'}}>
              Live Overview {results?`ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ${results.tickersDone}/${results.totalTickers} tickers`:'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ waiting...'}
            </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'14px'}}>
          {partial&&(
            <div style={{background:'#1c1a00',border:'1px solid #713f12',borderRadius:'6px',padding:'8px 14px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <span style={{color:'#fbbf24',fontSize:'11px'}}>ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ³ Partial ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ {results.tickersDone}/{results.totalTickers} tickers. Auto-updates every {CHUNK}.</span>
              <span style={{color:'#713f12',fontSize:'10px'}}>Switch to ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶ Running for progress</span>
            </div>
          )}

          {/* Summary table ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ all scenarios at once */}
          <div style={C.card}>
            <div style={{fontSize:'10px',color:'#6b7280',marginBottom:'10px',textTransform:'uppercase',letterSpacing:'1px'}}>All Scenarios ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Summary</div>
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:'11px'}}>
              <thead>
                <tr style={{borderBottom:'1px solid #1f2937'}}>
            ...
            <div style={{fontSize:'10px',color:'#374151',marginTop:'6px'}}>Click a row to drill into signal analysis. Total Return = flat sizing, sum of (winsÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂTP ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ lossesÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂSL).</div>
          </div>
            ...
            <>
              ...
              <div style={C.card}>
                <div style={{fontSize:'10px',color:'#6b7280',marginBottom:'8px',textTransform:'uppercase',letterSpacing:'1px'}}>
                  Scenario <span style={{color:SCENARIOS[scen].color}}>{scen}</span> ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Signal Tier Analysis
                ...
                {(scen==='G'||scen==='H')&&(
                  <div style={{fontSize:'10px',color:'#713f12',background:'#1c1200',padding:'6px 8px',borderRadius:'3px',marginBottom:'8px'}}>
                    ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¹ Gap-down setup: LOW Momentum = counter-trend gap = better reversal candidate. LOW Catalyst = no fundamental driver = gap may be overreaction.
                  </div>
                )}
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  <thead>
                    <tr>
                      ...
                      <th style={{...C.th,textAlign:'center',color:SCENARIOS[scen].color}}>ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ (HÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂL)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {r.sigStats.map(sig=>(
                ...
                <div style={{fontSize:'10px',color:'#374151',marginTop:'6px'}}>ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ = High tier WR ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Low tier WR ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· Green ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¥65% ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· Yellow ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¥52% ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· Red &lt;52% ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ = &lt;5 trades</div>
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
                <div style={C.card}>
                  <div style={{fontSize:'10px',color:'#6b7280',marginBottom:'8px',textTransform:'uppercase'}}>
                    Scenario {scen} ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Signal Predictive Power (ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ pp)
                  </div>
                  <ResponsiveContainer width="100%" height={160}>
                    <BarChart data={[...r.sigStats].sort((a,b)=>Math.abs(b.power)-Math.abs(a.power)).map(s=>({name:s.label,v:+(s.power*100).toFixed(0),color:s.color}))}
                      ...
                      <Tooltip formatter={v=>[`${v}pp`,'HighÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂLow spread']} contentStyle={{background:'#111827',border:'1px solid #1f2937',fontSize:'11px',color:'#e5e7eb'}}/>
                ...
                <div style={C.card}>
                  <div style={{fontSize:'10px',color:'#6b7280',marginBottom:'8px',textTransform:'uppercase'}}>Top Signal Pairs ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Both High Tier</div>
