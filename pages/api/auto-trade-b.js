...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (r.ok) { const d = await r.json(); const raw = d.quotes && d.quotes.quote; if (raw) { (Array.isArray(raw)?raw:[raw]).forEach(q=>{quoteMap[q.symbol]=q;}); } }
  } catch(_) {}
  ...
  for (const ticker of tickers) {
    ...
    try {
      const or = await fetch('https://paper-api.alpaca.markets/v2/orders', { method:'POST', headers:AH, body:JSON.stringify({ symbol:sym, side:'sell', type:'market', time_in_force:'day', qty:String(qty), order_class:'bracket', take_profit:{ limit_price:String(takeProfitPrice) }, stop_loss:{ stop_price:String(stopLossPrice) } }) });
      const o = await or.json();
      if (!or.ok) results.push({ symbol:sym, status:'error', reason:o.message||'Alpaca error', gap:+gap.toFixed(2), rvol_logged:rvol, tier });
      else results.push({ symbol:sym, status:'traded', scenario:'E', variant:'{variant}', side:'sell', qty, entryPrice:+price.toFixed(2), takeProfitPrice, stopLossPrice, tier, tierLabel:label, tpPct:tp, slPct:sl, gap:+gap.toFixed(2), rvol_logged:rvol, orderId:o.id });
    } catch(e) { results.push({ symbol:sym, status:'error', reason:e.message }); }
  }
  return res.status(200).json({ timestamp:new Date().toISOString(), variant:'B-tiered',
