...
export default async function handler(req, res) {
  ...
  // Thresholds from config with optimal defaults
  ...
  try {
    ...
    if (r.ok) {
      ...
      if (raw) {
        ...
        arr.forEach(q => { quoteMap[q.symbol] = q; });
      }
    }
  } catch (_) {}
  ...
  for (const ticker of tickers) {
    ...
    if (qty < 1) {
      results.push({ symbol: sym, status: 'skipped', reason: 'Bet too small', gap: +gap.toFixed(2), scenario: scenario.name, rvol_logged: rvol });
      continue;
    }

    // Long OTOCO: buy market entry, sell limit TP, sell stop SL
    ...
    try {
      ...
      if (!or.ok || (od.order && od.order.status === 'error')) {
        results.push({
          symbol: sym, status: 'error', scenario: scenario.name,
          reason: od.order?.partner_error_description || od.fault?.faultstring || `Tradier ${or.status}`,
          gap: +gap.toFixed(2), rvol_logged: rvol,
        });
      } else {
        results.push({
          symbol: sym, status: 'traded', scenario: scenario.name,
          side: 'buy', qty, entryPrice: +price.toFixed(2),
          priceSource: bidPrice ? 'bid' : 'last',
          tpPct: scenario.tpPct, slPct: scenario.slPct,
