// pages/api/auto-trade.js — Variant A: Tradier data + Alpaca execution
// Cron: 9:29 AM EDT weekdays (cron-job.org "APEX Auto-Trade A")
...
export default async function handler(req, res) {
  ...
  for (const ticker of tickers) {
    ...
    try {
      ...
      else results.push({ symbol:sym, status:'traded', scenario:'E', variant:'A-Tradier-data-Alpaca-exec', side:'sell', qty, entryPrice:+price.toFixed(2), takeProfitPrice:tp, stopLossPrice:sl, gap:+gap.toFixed(2), rvol_logged:rvol, orderId:o.id, orderStatus:o.status });
    } catch(e) { results.push({ symbol:sym, status:'error', reason:e.message }); }
  }
  return res.status(200).json({ timestamp:new Date().toISOString(), variant:'A-Tradier-data-Alpaca-exec', summary:{ traded:results.filter(r=>r.status==='traded').length, skipped:results.filter(r=>r.status==='skipped').length, errors:results.filter(r=>r.status==='error').length }, trades:results });
}