...
export default async function handler(req, res) {
  ...
  if (!config.scenarios || !config.scenarios.G) return res.status(200).json({ message: 'Scenario G disabled', trades: [] });
  ...
  let quoteMap = {};
  try { const r = await fetch(`${BASE}/markets/quotes?symbols=${symbols.join(',')}&greeks=false`, { headers: H }); if (r.ok) { const data = await r.json(); const raw = data.quotes && data.quotes.quote; if (raw) { const arr = Array.isArray(raw) ? raw : [raw]; arr.forEach(q => { quoteMap[q.symbol] = q; }); } } } catch(_) {}
  ...
  for (const ticker of tickers) {
    ...
    let entryPrice = null;
    try { const ts = await fetch(`${BASE}/markets/timesales?symbol=${sym}&interval=1min&start=${encodeURIComponent(startET)}&end=${encodeURIComponent(endET)}&session_filter=all`, { headers: H }); if (ts.ok) { const d = await ts.json(); const series = d.series && d.series.data; if (series) { const candles = Array.isArray(series) ? series : [series]; const fg = candles.find(c => parseFloat(c.close) > parseFloat(c.open)); if (fg) entryPrice = +parseFloat(fg.close).toFixed(2); } } } catch(_) {}
    ...
    const tp = +(entryPrice * 1.015).toFixed(2), sl = +(entryPrice * 0.980).toFixed(2);
    try {
      ...
      else results.push({ symbol:sym, status:'traded', scenario:'G', side:'buy', qty, entryPrice, takeProfitPrice:tp, stopLossPrice:sl, gap:+gap.toFixed(2), rvol, orderId:od.order?.id, orderStatus:od.order?.status });
    } catch(e) { results.push({ symbol:sym, status:'error', reason:e.message }); }
  }
  return res.status(200).json({ timestamp:new Date().toISOString(), variant:'G-Tradier-9:31', summary:{traded:results.filter(r=>r.status==='traded').length,skipped:results.filter(r=>r.status==='skipped').length,errors:results.filter(r=>r.status==='error').length}, trades:results });
}